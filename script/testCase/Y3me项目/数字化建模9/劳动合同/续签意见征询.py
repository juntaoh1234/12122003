...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="续签意见征询"]')
        ...
        assert u"续签意见征询" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('HRGXFW020070')
        # # driver.switch_to.frame(iframe)
        driver.switch_to.frame(iframe)
        sleep(2)
        # 搜索
        driver.find_element_by_xpath('//span[text()="新增"]').click()
        driver.find_element_by_xpath('//span[text()="续签征询"]/../../..//span[text()="取消"]').click()
