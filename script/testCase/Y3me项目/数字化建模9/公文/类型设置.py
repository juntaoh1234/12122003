...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="类型设置"]')
        ...
        assert u"类型设置" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('XTGONGWEN0021')
        driver.switch_to.frame(iframe)

        # 新建类型
        driver.find_element_by_xpath('//span[text()="新建类型"]').click()
