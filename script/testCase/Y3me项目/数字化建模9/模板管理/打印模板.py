...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(5)
        # 进入一级节点
        menu2 = driver.find_element_by_css_selector('span[title="模板管理"]')
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="打印模板"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"UI模板" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('GZTTMP040')
        driver.switch_to.frame(iframe)
        sleep(3)
        # 新增空白模板
        temp = driver.find_element_by_xpath('//*[@class="add-temp"]')
        ActionChains(driver).move_to_element(temp).perform()
        driver.find_element_by_xpath('//span[text()="新增空白模版"]').click()
        # 取消
