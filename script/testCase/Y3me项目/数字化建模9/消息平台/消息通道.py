...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="消息通道"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"消息通道" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('GZTMSG010')
        driver.switch_to.frame(iframe)
        sleep(3)
        # 添加模板
        driver.find_element_by_xpath('//p[text()="邮箱"]').click()
        # 编辑
        ad_button = driver.find_element_by_xpath('//button[text()="编辑"]')
