...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="审批常用语"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"审批常用语" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('XTLCZX0011')
