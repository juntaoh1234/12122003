...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="数据填报"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"数据填报" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('intellivworkbenchplatform-23')
        driver.switch_to.frame(iframe)
        sleep(3)
        # 新建填报
        new_button = driver.find_element_by_xpath('//*[text()="新建填报"]')
