...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="咨询列表"]')
        ...
        assert u"咨询列表" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('0351')
        driver.switch_to.frame(iframe)

        #全选
        driver.find_element_by_xpath('//label[@class="checkbox-inline"]/input[@ng-model="isAllChecked"]').click()
        sleep(3)
