...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="商品分析"]')
        ...
        assert u"商品分析" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('0802')
        driver.switch_to.frame(iframe)

        # 点击昨日
        driver.find_element_by_xpath('//div[@class="col-xs-12 text-right m-b-15"]/a[text()="昨日"]').click()
        sleep(3)
