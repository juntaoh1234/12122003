...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="应收结账"]')
        ...
        assert u"应收结账" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('marap_ar_settle')
        driver.switch_to.frame(iframe)

        # 选择会计主体
        driver.find_element_by_xpath('//span[text()="会计主体"]/../..//i').click()
        driver.find_element_by_xpath('//*[text()="yontest"]').click()
        driver.find_element_by_xpath('//*[text()="确 定"]').click()
        # 结账检查
        driver.find_element_by_xpath('//*[text()="结账检查"]').click()
        sleep(3)
