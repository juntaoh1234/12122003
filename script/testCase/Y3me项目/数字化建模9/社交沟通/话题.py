...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="话题"]')
        ...
        assert u"话题" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('XTHUATI00000000000')
        driver.switch_to.frame(iframe)

        # 创建话题
        driver.find_element_by_xpath('//span[text()="＋创建话题"]').click()
        driver.find_element_by_xpath('//input[@class="topicz-input"]').send_keys("抢票了")
