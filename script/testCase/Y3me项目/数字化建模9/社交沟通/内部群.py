...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="内部群"]')
        ...
        assert u"内部群" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('XTNEIBUQUN0001')
        driver.switch_to.frame(iframe)

        # 创建内部群
        driver.find_element_by_xpath('//div[text()="+ 创建内部群"]').click()
        sleep(3)
        # 取消
