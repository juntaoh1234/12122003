...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="员工变动分析"]')
        ...
        assert u"员工变动分析" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('HRPA080')
        driver.switch_to.frame(iframe)

        # 点击入职分析
        driver.find_element_by_xpath('//*[text()="入职分析"]').click()
