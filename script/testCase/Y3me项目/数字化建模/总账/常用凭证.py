...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(1)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="常用凭证"]')
        ...
        assert u"常用凭证" in titleName, u"页面源码中不存在该关键字！"
        sleep(2)
        iframe = driver.find_element_by_id('cypz')
        ...
        code = '模板编码{}'.format(num)
        driver.find_element_by_xpath('//label[text()="模板编码"]/following-sibling::div//input').send_keys(code)

        # 输入摘要
        num = random.randint(999, 10000)
        message = '模板名称{}'.format(num)
        driver.find_element_by_xpath('//label[text()="模板名称"]/following-sibling::div//input').send_keys(message)

        # 点击期末结转凭证按钮
        driver.find_element_by_xpath('//label[text()="期末结转凭证"]/following-sibling::input').click()

        # 输入凭证类型
        driver.find_element_by_xpath(
            '//label[text()="凭证类型"]/following-sibling::div//span[@class="input-group-addon cursor-style"]').click()
        sleep(0.5)
        # 选择第一条
        driver.find_element_by_xpath(
            '//ul[@class="dropdown-menu bootstrap-typeahead-menu dropdown-menu-justify"]/li[1]').click()
        # 选择检查策略
        driver.find_element_by_xpath('//label[text()="结账检查项目策略"]/following-sibling::div//div/div').click()
        sleep(1)
        # 选择一条
        driver.find_element_by_xpath('//li[text()="警告"]').click()

        # 输入描述信息
        driver.find_element_by_xpath('//textarea').send_keys('描述信息99999999')
        ...
        self.assertNotIn(message, driver.page_source)
