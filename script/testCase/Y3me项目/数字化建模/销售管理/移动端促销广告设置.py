...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="移动端促销广告设置"]')
        ...
        assert u"移动端促销广告设置" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('appPromotionAdsShow')
        driver.switch_to.frame(iframe)
        
        # 新增
        driver.find_element_by_xpath('//span[contains(text(),"新增")]').click()
        driver.find_element_by_xpath('//div[@class="ivu-row"]//span[contains(text(),"取消")]').click()
