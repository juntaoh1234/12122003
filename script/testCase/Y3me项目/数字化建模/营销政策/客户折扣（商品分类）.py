...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="客户折扣（商品分类）"]')
        ...
        assert u"客户折扣（商品分类）" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('agentSalesProductClassShow')
        driver.switch_to.frame(iframe)

        # 新增
        driver.find_element_by_xpath('//span[contains(text(),"新增")]').click()
        # 取消
        driver.find_element_by_xpath('//*[@id="single-page-right"]//span[contains(text(),"取消")]').click()
        # 确定
