...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="评论列表"]')
        ...
        assert u"评论列表" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('0341')
        driver.switch_to.frame(iframe)

        # 添加评论
        driver.find_element_by_id('impor').click()
        driver.find_element_by_xpath('//li[text()="单条添加"]').click()
        sleep(3)
