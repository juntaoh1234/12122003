...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(1)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="重新生成凭证"]')
        ...
        assert u"重新生成凭证" in titleName, u"页面源码中不存在该关键字！"
        sleep(2)
        iframe = driver.find_element_by_id('tovoucher')
        driver.switch_to.frame(iframe)

        # 搜索
        new_button = driver.find_element_by_xpath('//button[text()="搜索"]')
