...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="外部单位设置"]')
        ...
        assert u"外部单位设置" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('XTGONGWEN0024')
        driver.switch_to.frame(iframe)

        # 新建机构组
        driver.find_element_by_xpath('//span[text()="新建外部单位"]').click()
