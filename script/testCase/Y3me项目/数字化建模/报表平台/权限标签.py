...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="权限标签"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"权限标签" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('intellivworkbenchplatform-24')
        driver.switch_to.frame(iframe)
        sleep(3)
        # 添加
        new_button = driver.find_element_by_xpath('//*[text()="添 加"]')
        driver.execute_script("arguments[0].click();", new_button)

        # 取消
        sd_button = driver.find_element_by_xpath('//*[text()="取 消"]')
