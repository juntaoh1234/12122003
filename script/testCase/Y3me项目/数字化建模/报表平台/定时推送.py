...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="pureBottom bottomBar"][title="定时推送"]')
        actions.move_to_element(menu3)
        actions.click(menu3)
        actions.perform()

        assert u"定时推送" in driver.page_source, u"页面源码中不存在该关键字！"
        sleep(3)
        # 切换iframe
        iframe = driver.find_element_by_id('intellivworkbenchplatform-25')
        driver.switch_to.frame(iframe)
        sleep(3)
        # 新建数据模型
        new_button = driver.find_element_by_xpath('//button/span[text()="新建任务"]')
