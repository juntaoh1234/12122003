...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="查询统计"]')
        ...
        assert u"查询统计" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('service_xtyyjm004')
        driver.switch_to.frame(iframe)

        # 新建查询
        driver.find_element_by_class_name('addNew').click()
        # 取消
        driver.find_element_by_xpath('//div[contains(text(),"新建")]/../..//span[contains(text(),"取消")]').click()
