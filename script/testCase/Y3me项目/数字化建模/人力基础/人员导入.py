...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="人员导入"]')
        ...
        assert u"人员导入" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('HRSZ030010')
        driver.switch_to.frame(iframe)

        # 新增
        driver.find_element_by_xpath('//div[@class="control-btn"]//span[text()="数据导入"]').click()
        driver.find_element_by_xpath('//span/button[1]/span[text()="取消"]').click()
