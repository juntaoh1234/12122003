...
class EasyCase(TestCase):
    ...
    def runTest(self):
        ...
        sleep(3)
        # 进入二级节点
        menu3 = driver.find_element_by_css_selector('li[class="bottomBar"][title="仓库物料关系"]')
        ...
        assert u"仓库物料关系" in titleName, u"页面源码中不存在该关键字！"
        sleep(5)
        iframe = driver.find_element_by_id('st_wareandinvlist')
        driver.switch_to.frame(iframe)

        # 新增
        search_button = driver.find_element_by_xpath('//*[text()="新增"]')
        driver.execute_script("arguments[0].click();",search_button)
        driver.find_element_by_xpath('//*[text()="取 消"]').click()
